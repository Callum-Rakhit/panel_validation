...
for i in *R1_001.fastq.gz; do mv $i "${i%_L00*}_R1_001.fastq.gz"; done
for i in *R2_001.fastq.gz; do mv $i "${i%_L00*}_R2_001.fastq.gz"; done
...
for i in *$FILENAMESUFFIX1; do mv $i "${i%_R1_001.fastq.gz}"; done

# Place the relevant R2 files in the relevant directories
for i in *$FILENAMESUFFIX2; do mv $i "${i%_R2_001.fastq.gz}"; done
