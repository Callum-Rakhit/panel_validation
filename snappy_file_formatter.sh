...
FILENAMESUFFIX1="_L001_R1_001.fastq.gz"
FILENAMESUFFIX2="_L001_R2_001.fastq.gz"

# list unique sample names alongside R1/R2 status
for i in *R1_001.fastq.gz; do cat $i >> "${i%_L00**}_L001_R1_001.fastq.gz"; done
for i in *R2_001.fastq.gz; do cat $i >> "${i%_L00**}_L001_R2_001.fastq.gz"; done

# Make a directory for the sample
for i in *$FILENAMESUFFIX1; do mkdir "${i%_L001_R1_001.fastq.gz}"; done

# Place the relevant R1 files in the relevant directories
for i in *$FILENAMESUFFIX1; do mv "$i" "${i%_L001_R1_001.fastq.gz}"; done

# Place the relevant R2 files in the relevant directories
for i in *$FILENAMESUFFIX2; do mv "$i" "${i%_L001_R2_001.fastq.gz}"; done
